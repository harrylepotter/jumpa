...
{
	//NSLog(@"Proximitymanager: initWithSize");
  self = [super init];
  if (self)
  {
    gridSize = gs;
...
{
	////NSLog(@"Proximitymanager: addObject");
  [objects addObject:object];
}

-(void)resetVisibility{
	NSLog(@"Proximitymanager : resetVisibility");
	for(int i=0; i< [objects count]; i++){
		[[objects objectAtIndex:i] setVisible:YES];
	}
}
...
-(NSMutableArray*)removeCachedNode:(CocosNode*)thenode
{
	uint off = gridSize * 1024;

	uint index = (int)((thenode.position.x + off) / gridSize) << 11 | (int)((thenode.position.y + off) / gridSize); // max of +/- 2^10 rows and columns
	uint cells = 1 + (1 / gridSize);
	...
	if (r){
		...
		[cachedPositions removeObjectForKey:tmp];
		[positions removeObjectForKey:tmp];
	}
