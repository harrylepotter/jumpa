...
- (void)setRolloff:(float)factor{
	//NSLog(@"%@ : setRolloff: %f", file, factor);
...
-(void)seek:(float)seconds{
	//NSLog(@"seek %f seconds", seconds);
	ALint sample_freq;
	alGetBufferi(buffer, AL_FREQUENCY, &sample_freq);
	alSourcef(source, AL_SAMPLE_OFFSET, sample_freq*seconds);	
}

// play messages
- (void)playAtPosition:(CGPoint)p restart:(BOOL)r {
	//NSLog(@"PASoundsource (%@) : playAtPosition: (%f,%f)", file, p.x,p.y);
...
- (void)setPosition:(CGPoint)pos {
	//NSLog(@"pasoundsource (%@) : setPosition: %f,%f", file, pos.x, pos.y);
    float x,y;
//    position = pos;
...
//	alSourcefv(source, AL_POSITION, sourcePosAL);
	...
	//NSLog(@"Setting sound position : %f,%f", x,y);
    float sourcePosAL[] = {x, y, 0.0f};
	alSourcefv(source, AL_POSITION, sourcePosAL);
}
